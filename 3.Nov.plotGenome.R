...
findArgument <- function(pair,pval,canDir,norDir,extend,outpref,drec,d){
	canFile = c()
	norFile = c()
	apa = c()
	out = c()
	...
	for(i in 1:dim(pval)[1]){
		...
		out = c(out,paste(outpref,"_",rownames(pval)[i],sep=""))
...
drec_s = drec[rowSums(drec=="shorten")>=minDynamicNum,]
pval_l = pval[rowSums(drec=="lengthen")>=minDynamicNum,]
drec_l = drec[rowSums(drec=="lengthen")>=minDynamicNum,]

short_arg = findArgument(pair,pval_s,canDir,norDir,extend,paste(outpref,"_short",sep=""),drec_s,"shorten")
long_arg = findArgument(pair,pval_l,canDir,norDir,extend,paste(outpref,"_long",sep=""),drec_l,"lengthen")
