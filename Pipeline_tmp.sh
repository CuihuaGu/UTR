...
echo `date` >> $logFile
Rscript /data/guc1/UTR/Code/21.Sep.CalcDiff.R ${outDir}/ReadCounts/${outPref}.TdTpNdNp.txt ${pCutoff} ${numCutoff} ${diffDir} ${fcCutoff} ${filterFile}

# 4. 21.Sep.plotGenome.R pvalFile drecFile 1 outpref 2 apaPair 9
echo -------------Genome visualization >> $logFile
echo `date` >> $logFile
#Rscript /data/guc1/UTR/Code/4.Nov.plotGenome.R ${diffDir}.NA.pval.txt ${diffDir}.NA.drec.txt ${pairFile} ${picDir} ${canDir} ${norDir} ${extend} ${minDynamicNum} ${outDir}/CalcDiff/${outPref}_1e-06.2fold_short.txt ${outDir}/CalcDiff/${outPref}_1e-06.2fold_long.txt ${pCutoff}
